...
Orificeloss=13.61 #loss due to addition of orifice(atm)
PressureLimit=98.07
...
Orifice=np.empty(X.shape)
Orifice_loss=np.zeros(X.shape) 
...
for d in X :
    ...
    P[i]=P_inital-FLoss[i]-RHO*g*Z[i]*PaToAtm*1000+WPump-OrificeWork
    ...
    if P[i]<1.3 :
        ...
        P[i]=P_inital-FLoss[i]-RHO*g*Z[i]*PaToAtm*1000+WPump-OrificeWork
        Pump_Work[i]=68.046
    if P[i]>(PressureLimit-0.5):
        OrificeWork+=13.61
        Orifice[i]=P[i]
        P[i]=P_inital-FLoss[i]-RHO*g*Z[i]*PaToAtm*1000+WPump-OrificeWork
        Orifice_loss[i]=13.61
    i+=1
...
plt.plot(X,Pump,"Hr")
plt.plot(X,Orifice,"bo")
plt.xlabel("Pipe Length (Km)")      #setting labels
plt.ylabel("Pressure Inside Pipe (atm)")#setting labels
plt.title("PRESSURE INSIDE PIPE(0.562'Thick Grade 65) WITH PUMPS ON ACTUAL TERRAIN") #setting labels
plt.legend(["Pressure Profile","Pump Locations","Orifice Location"])
ax2 = plt.gca().twinx()
ax2.plot(X, Z,color="gray",linestyle="dashed", label="ELEVATION (In KM)")
ax2.set_ylabel("ELEVATION (KILOMETER)", color="gray")
ax2.tick_params(axis="y", labelcolor="gray")
ax2.set_ylim((0,1.6))
ax2.legend(["ELEVATION"],loc="lower right")
plt.show()  # Show the graph


# # Sheet export
# data=np.vstack((X,Z,FLoss,ElevationLoss,Pump,Pump_Work,Orifice,Orifice_loss,P))
# df=pd.DataFrame(data.transpose(),columns=["Pipe Length (Km)","Elevation(Km)","Friction Pressure Loss (atm)","Elevation Loss (atm)","Pump","Pump Work(atm)","Orifice","Orifice Loss","Resultant Pressure (atm)"])
# df=df.fillna(0)
# df["Pump"]=df["Pump"].where(df["Pump"]==0,"ONE PUMP ADDED")
# df["Orifice"]=df["Orifice"].where(df["Orifice"]==0,"ONE ORIFICE ADDED")
# print(df)
# df.to_excel("RealPipeGrade70D562.xlsx",index=False)